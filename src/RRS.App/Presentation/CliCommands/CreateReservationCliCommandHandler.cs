...
internal class CreateReservationCliCommandHandler : ICliCommandHandler
{
    ...
    public CreateReservationCliCommandHandler(ILogger<CreateMeetingRoomCliCommandHandler> logger, IMediator mediator, IValidator<CreateReservationCommand> validator,
    ...
    {
        _logger.LogDebug($"{nameof(CreateReservationCliCommandHandler)} cli command received.");
        ...
        var createReservationCommand = new CreateReservationCommand(
           ...
           Environment.UserName,
            (DateTime)options[StartKey]!,
            (DateTime)options[EndKey]!
        );

        var result = await _validator.ValidateAsync(createReservationCommand);
        if (!result.IsValid)
        {
